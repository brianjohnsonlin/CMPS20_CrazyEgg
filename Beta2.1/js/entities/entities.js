	...
	update: function() {
		...
		if(this.state == "normal"){
			...
			if (res) {
				...
				if (res.obj.type == "rampup" && this.alive){
					this.vel.y = -0.6;
					this.gravity = 0;
					this.falling = false;
				}
				...
				if(res.obj.type == "riseup" && this.alive){
					this.vel.y = -5;
					this.gravity = 0;
					this.falling = false;
				}
				if(res.obj.type == "ciderslide" && this.alive)this.state = "slide";
				...
				if(res.obj.type == "pool" && this.alive){
				...
				if(res.obj.type!=null) if((res.obj.type.substring(0,4)=="area" ||
				   res.obj.type.substring(0,5)=="scene") && this.alive &&
				   this.state!="lvlchng"){
